...
export function recordExamplesFlex() {
  ...
  return bubble('更多紀錄範例：點例句，改好再送出', {type:'carousel',contents:pages.map(([title,groups],i)=>({
    ...
    body:{type:'box',layout:'vertical',paddingAll:'16px',backgroundColor:BODY_BG,contents:[
      text('點例句，補上數量或改好內容再送出',{size:'16px',wrap:true,color:C.inkSoft}),
      ...groups.map(([label,examples])=>({type:'box',layout:'vertical',margin:'12px',spacing:'6px',contents:[
        ...
        ...examples.map(example=>({type:'box',layout:'horizontal',paddingAll:'10px',spacing:'12px',alignItems:'center',backgroundColor:C.tint,cornerRadius:'10px',action:{type:'postback',label:example.replace(/\n/g,'＋'),data:'action=fill',inputOption:'openKeyboard',fillInText:example},contents:[
