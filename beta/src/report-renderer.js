...
export function imageDocument(snapshot, url) {
  const html=baseImageDocument(snapshot.purpose==='doctor'&&snapshot.doctorSource?{...snapshot,sections:[],notice:'',empty:false}:snapshot,url);
...
export function paginateImages() {
  ...
  if(!source.textContent.trim())return 0;
  let page;
