...
export function imageDocument(snapshot, url) {
  if(snapshot.purpose!=='doctor'||!snapshot.doctorSource)return baseImageDocument(snapshot,url);
  // Doctor output is the existing A4 document plus QR only. Do not build the
  // separate text-summary template/source/pagination host for this mode.
  const html=baseImageDocument(snapshot,url,false);
  const built=buildA4Report(doctorReportData(snapshot));
  return html.replace('</style>',`${reportA4Css}\n.a4-page{width:760px;min-height:0;overflow:visible;font:14px/1.4 sans-serif;--serif:serif}.a4-page section{padding:9px 11px 10px}.a4-page h2{font-size:17px}.a4-page p{white-space:normal}</style>`).replace('<article class="page qr"',built.html.replaceAll('class="a4-page"','class="a4-page page"')+'<article class="page qr"');
}
function baseImageDocument(snapshot, url, includeSummary=true) {
  const qr=qrcode(0,'M'); qr.addData(url,'Byte'); qr.make();
  const blocks=(includeSummary?snapshot.sections:[]).flatMap(section => section.items.flatMap((item,index) => {
...
export function paginateImages() {
  ...
  if(!host||!source||!source.textContent.trim())return 0;
  let page;
