...
- 本機完整測試 604 項通過；先前的 570 是交接時基準。
- 高頻操作 MVP：記一筆及完成確認卡使用六類型 Quick Reply（主食／乾乾／水／藥／尿尿／便便）。類型按鈕不再帶入歷史份量或用藥結果；數量沿用 parser／amount 等待、多貓沿用 pendrec。完整現況、後台依賴及 25 項驗收見 `HIGH_FREQUENCY_UX.md`；本輪不做後台隱藏或另一套 Rich Menu。
...
- **LINE 出報告目前流程**：先選貓，再選給醫生／給照護者，直接回報告 PNG（長文分頁）＋ QR Code PNG ＋閱讀連結，不再把出報告入口導向 LIFF 後台。醫生固定近 14 天；照護沿用該貓設定／範本。缺少餵食安排時在 LINE 回文字、確認後存範本；也可明確選擇只出已有資料（會標示缺漏）。
