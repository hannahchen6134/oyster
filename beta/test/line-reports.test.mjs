...
test('照護缺少餵食安排在 LINE 補填，確認後存該貓範本才出圖',()=>setup(async({db,env,sent,event,post,flow})=>{
 ...
 await handleLineReportText(env,event,'owner','早晚餵主食罐40g，水補滿');assert.equal((await flow()).stage,'confirm');assert.equal(await appKvGet(db,'careTemplate:owner:p1'),null);
...
test('現有照護範本直接出圖；多頁全數分批傳回，重點擊不重跑',()=>setup(async({db,event,env,sent,post,flow})=>{
 await appKvSet(db,'careTemplate:owner:p1',JSON.stringify({draft:{feeding:'依已確認安排餵食'},updatedAt:new Date().toISOString()}));
