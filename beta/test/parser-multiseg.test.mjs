...
test('multiRecord 不靜默丟棄：食物段無法解析時保留在 unparsed、水正常記、數量不交換', () => {
  for (const s of ['罐頭皇家34g 水14g', '皇家罐頭34g水14g', '罐頭皇家34g，水14g', '罐頭皇家34g\n水14g']) {
    ...
    assert.ok(Array.isArray(r.unparsed) && r.unparsed.length >= 1, `${s} 應保留未解析片段`);
    assert.ok(r.unparsed.join('').includes('34'), `${s} 未解析片段應含食物 34`);
...
test('保守連接詞：然後/接著 後接事件詞才分隔；再＋喝水＝兩筆', () => {
  ...
  assert.ok(r1.unparsed.join('').includes('34')); // 皇家罐頭34 尚未記錄
