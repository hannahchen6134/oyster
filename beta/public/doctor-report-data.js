...
export function cleanDoctorSource(source={}){
 const rows=(Array.isArray(source.rows)?source.rows:[]).slice(0,90).map(r=>({date:String(r.date||'').slice(0,10),...Object.fromEntries(['entryCount','waterMl','foodWaterMl','totalWaterMl','dryFoodG','wetFoodG','otherFoodG','kcal','kcalEstimated'].map(k=>[k,number(r[k])])),kcalIncomplete:!!r.kcalIncomplete}));
...
export function doctorReportData(snapshot){
 ...
 for(let i=0;i<90&&Number.isFinite(+dt);i++,dt.setUTCDate(dt.getUTCDate()+1)){const date=dt.toISOString().slice(0,10);if(date>s.to)break;dates.push(date);}
 ...
 daily:dates.slice().reverse().map(date=>{const r=byDate.get(date);return r?.entryCount>0?{date,waterMl:r.totalWaterMl,wetG:r.wetFoodG+r.foodWaterMl,dryG:r.dryFoodG,kcal:r.kcal,kcalEstimated:!!r.kcalEstimated,kcalIncomplete:r.kcalIncomplete}:{date,unrecorded:true};})};
}
