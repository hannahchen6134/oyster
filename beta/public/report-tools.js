...
export async function qrPng(url, label) {
  ...
  ctx.drawImage(image,48,100+extra,704,704);ctx.font='24px sans-serif';ctx.fillText('掃碼查看這份摘要',400,850+extra);ctx.font='20px sans-serif';ctx.fillText('喵喵管家 · 有效期限以摘要頁為準',400,898+extra);
...
export function carePrefill(draft,esc) {
  ...
  return `<div class="care-prefill-head"><h2>已幫你帶入</h2><span>${values.length} 項資料</span></div>${values.length?`<details><summary>查看已帶入的內容</summary><dl>${values.map(([key,label])=>`<div><dt>${label}</dt><dd>${esc(draft[key])}</dd></div>`).join('')}</dl></details>`:'<p class="muted">目前還沒有照護設定，可以在下方補充一次，之後存成範本沿用。</p>'}<button id="careEditOpen" class="link-btn back-link" type="button">查看／修改內容 →</button>`;
}
